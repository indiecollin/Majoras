...
const thickness = 2;
const dashGap = 10;
const color = `${notebookBackgroundPattern} 25%, #0000 0`

const NotebookContainer = styled.div`  
    background: 
        conic-gradient(at ${dashGap}px ${thickness}px, ${color})
        calc((${gridSize}px/${granularity} - ${dashGap}px + ${thickness}px)/2) 0/
        calc(${gridSize}px/${granularity}) ${gridSize}px,
        conic-gradient(from 180deg at ${thickness}px ${dashGap}px, ${color})
        0 calc((${gridSize}px/${granularity} - ${dashGap}px + ${thickness}px)/2)/
        ${gridSize}px calc(${gridSize}px/${granularity});
